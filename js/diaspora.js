    ...
    $('body').on('click', function(e) {
        ...
        switch (true) {
            ...
            case (tag.indexOf('more') != -1):
                ...
                tag.html('読み込み中‥').data('status', 'loading')
                Diaspora.loading()
                Diaspora.L(tag.attr('href'), function(data) {
                    var link = $(data).find('.more').attr('href');
                    if (link != undefined) {
                        tag.attr('href', link).html('もっと読み込む').data('status', 'loaded')
                    ...
                    tag.html('もっと読み込む').data('status', 'loaded')
                })
                return false;
                break;
            // home
            ...
            case - 1 != tag.indexOf("comment"):
				if($('#gitalk-container').data('enable') == true){
					...
					$('#gitalk-container').html("コメントを締め切りました");
				}
                return false;
                break;
            default:
